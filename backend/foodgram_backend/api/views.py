...
from recipes.models import User, Subscriptions, Tag, Ingredient, Recipe, Favorite, ShoppingCart
...
from .serializers import TagSerializer, RecipeSerializer, IngredientSerializer, FavoriteSerializer, LimitedRecipeSerializer, ShoppingCartSerializer
from .permissions import IsPostOrReadOnly, IsAdminOrSelf, IsAdminUserOrReadOnly
...
class IngridientViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Ingredient.objects.filter(
            recipeingredient__ingredient_id=self.kwargs.get('id'))
...
class RecipeViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        if not self.request.user.is_authenticated:
            raise PermissionDenied("Only authenticated users can create recipes.")
...
class FavoriteViewSet(ModelViewSet):
    ...
    def create(self, request, recipe_id=None):
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save(user=request.user)  
        user_serializer = LimitedRecipeSerializer(recipe, many=False, context={'request': request})
        return Response(user_serializer.data, status=status.HTTP_201_CREATED)
...
class DownloadShoppingCartViewSet(ModelViewSet):
    ...
    def download(self, request):
        ...
        content = "\n".join([f"{name} - {amount} г" for name, amount in ingredients.items()])
